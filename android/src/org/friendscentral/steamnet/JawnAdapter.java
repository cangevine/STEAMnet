...
public class JawnAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
    	...
    	if (getJawns()[position].getType() == "S".charAt(0)) {
	    	Spark spark = (Spark) getJawns()[position];
    		...
    		Idea idea = (Idea) getJawns()[position];
	    	View contentView = null;
	    	LinearLayout ideaInfo = new LinearLayout(mContext);
	    	
	    	int numSparks = idea.getSparkArray().length;
    ...
    public void removeAtPosition(int pos) {
    	if (getJawns().length >= 1) {
	    	Jawn[] newSet = new Spark[getJawns().length - 1];
	    	for (int i = 0; i < pos; i++) {
	    		newSet[i] = getJawns()[i];
	    	}
